...
function validateWifi(){if(selectedConnection()==='ethernet')return true;if(effectiveSsid())return true;var e=document.getElementById('wifi_err');var m=document.getElementById('wifi_ssid');if(e)e.classList.add('show');if(m){m.classList.add('invalid');m.focus();m.scrollIntoView({block:'center',behavior:'smooth'});}return false;}
...
bool SmartHomeShopSetupPortal::save_from_request_(AsyncWebServerRequest *request) {
  ...
  const bool wifi_required = this->bounded_arg_(request, "connection_mode", 16) != "ethernet";

  // A Wi-Fi network is mandatory unless the user picked Ethernet. Reject the
  // submit without changing anything so the form can be corrected.
  if (wifi_required && ssid.empty()) {
